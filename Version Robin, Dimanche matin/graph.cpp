...
GraphInterface::GraphInterface(int x, int y, int w, int h)
{
    ...
    m_main_image.set_pic_name("fond.jpg");

    m_tool_box.add_child(m_add_box);
    m_add_box.set_dim(95,95);
    m_add_box.set_gravity_xy(grman::GravityX::Left, grman::GravityY::Down);
...
Graphe Graph::update(Graphe first)
{
    int n,e,f;
    if (!m_interface)
        return first;
    ...
    if(m_interface->m_add_box.get_value())
    {
        ...
        std :: cin >> n;
        add_interfaced_vertex(first.m_sommet[n].m_num, first.m_sommet[n].m_quantite, first.m_sommet[n].pos_x,first.m_sommet[n].pos_y, first.m_sommet[n].m_images);
...
/// test : on a bien des éléments interfacésb
    if (m_interface && remed.m_interface)
    {
